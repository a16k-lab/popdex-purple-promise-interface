...
async function fetchPopdexApi(endpoint: string): Promise<any> {
  ...
  const resp = await fetch(url, {
    headers: {
      ...
      'User-Agent': 'PopDEX-PinkyPromise-Tracker/1.0',
    },
  });
  if (!resp.ok) {
...
async function handleWalletVolumeRequest(req: IncomingMessage, res: ServerResponse) {
  try {
    const parsedUrl = new URL(req.url || '', `http://${req.headers.host || 'localhost'}`);
    const address = (parsedUrl.searchParams.get('address') || '').trim().toLowerCase();
    const rawStart = Number(parsedUrl.searchParams.get('startTs')) || 1789344000000;
    const rawEnd = Number(parsedUrl.searchParams.get('endTs')) || 1789948800000;
    ...
    if (!address || !address.startsWith('0x') || address.length !== 42) {
      ...
      res.end(JSON.stringify({ error: 'Invalid EVM address format' }));
      return;
    }
      ...
      configureServer(server) {
        server.middlewares.use((req, res, next) => {
          if (req.url && req.url.startsWith('/api/wallet-volume')) {
            handleWalletVolumeRequest(req, res);
          } else {
            next();
          }
        });
